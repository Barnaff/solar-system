...
**Time.** A slider scrubs through a selectable window — twelve hours to
±250 years — and the whole system reconfigures continuously. Play forward or
...
**Standing on a world.** Every planet, plus Pluto and seven major moons — 17
...
solar time, the Sun's altitude and azimuth, and the next sunrise and sunset —
...
them off at the horizon. Uranus is the one to try — tipped 97.8°, a site at
88°N gets a Sun that simply never sets, the equator gets a 17-hour day, and the
...
**Scale.** Every radius is the real one — equatorial, at the 1-bar level for
the giants — and the default size factor is 1, so nothing is exaggerated out of
the box. That does mean planets are genuinely sub-pixel from across the system,
so small bodies also carry a glow marker; a slider exaggerates size up to 2000×
...
is the honest way to check: the Sun must subtend 0.533° from Earth.

Sunlight falls off as 1/r², so exposure tracks the focused body's distance
...
| --- | --- | --- |
| Planet positions | JPL Keplerian elements (Standish), tables 1 and 2 | arcseconds 1800–2050; better than a degree 3000 BC–3000 AD |
| Moon position | ELP-2000/82 truncation (Meeus ch. 47), precessed to J2000 | ~10″ longitude, ~4″ latitude |
...
| Time scale | Espenak & Meeus ΔT, with observed IERS values 1970–2026 | sub-second in the modern era |
| Radii | IAU/NASA reference values, equatorial | exact to the published figure |

`npm run verify` runs 126 assertions against independently known events —
...
must be over longitude 0.000° to within 0.25° (one minute of rotation), and on
3 November it must be 4.10° past it. Those pass, which is what makes derived
quantities trustworthy — sunrise at Greenwich on 28 July 2026 comes out at
...
0.09°, having been 0.3° apart before the lunar precession reduction was added.
...
- Planetary positions come from Keplerian fits, not numerical integration.
  ...
  table 1 does not model the great Jupiter–Saturn inequality.
...
```
js/astro/     ephemerides — no rendering code, no three.js
...
The astronomy layer is deliberately free of any renderer dependency — it deals
...
quantises to about half a metre — more than the eye height you are standing at.
...
the horizon is 4.65 km away and subtends 0.042°, so a uniform 256-segment globe
...
| `Space` | Play / pause |
| `←` `→` | Step one unit of the current rate; `Shift` for ten |
| `↑` `↓` | Faster / slower |
